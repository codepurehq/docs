...
<Card title="Reference Details" icon="book-open" iconType:="solid">
  ...
  **Related CWEs:** CWE-829 (Untrusted Control Sphere), CWE-345 (Insufficient Data Verification)
...
<Tabs>
  <Tab title="Python">
    #### Framework Context
    This is less common in Python frameworks themselves but can occur in deployment scripts or custom application logic that fetches resources or code dynamically. The primary risk is often client-side (in Django/Flask templates).
  ...
  <Tab title="Java">
    #### Framework Context
    Primarily client-side risk in templates (JSP, Thymeleaf) missing SRI. Server-side risk involves custom auto-update logic or insecure class loading from remote sources (`URLClassLoader`).
    ...
    * **Server-Side:** **Avoid loading code from remote URLs** (`URLClassLoader` from untrusted sources). Download dependencies securely via build tools (Maven, Gradle) which support hash/signature verification. If auto-updates are essential, download over HTTPS and validate digital signatures (e.g., using `java.security.Signature`) and hashes against a secure manifest.
    ...
    public Object loadPluginFromLocal(String pluginJarName) throws Exception {
        ...
        if (!isAllowedPlugin(pluginJarName)) {
  ...
  <Tab title=".NET(C#)">
    ...
    #### Mitigation and Best Practices
    * **Client-Side:** Use ASP.NET Core's built-in **Tag Helpers for SRI** (`asp-append-version`, `asp-src-include`, `asp-fallback-href-include` etc. don't add SRI, but `asp-href-include` or `asp-src-include` with a CDN path can be paired). Manually add `integrity="..."` and `crossorigin="anonymous"` attributes.
    * **Server-Side:** **Strongly name** assemblies and verify signatures before loading. Download updates over HTTPS, check hashes, and verify digital signatures (`Authenticode`) using .NET APIs before using `Assembly.Load()`. Load assemblies only from trusted, access-controlled directories.
    ...
    <head>
      ...
      <script src="[https://cdnjs.cloudflare.com/ajax/libs/angular.js/1.8.2/angular.min.js](https://cdnjs.cloudflare.com/ajax/libs/angular.js/1.8.2/angular.min.js)"
              asp-fallback-src="~/lib/angular/angular.min.js"
              asp-fallback-test="window.angular"
              integrity="sha512-7oYXeK0OxTFxndh0erL8FsjGvrl2VMDor6fVqzlLGfwOQQqTbYsGPv4ZZ15QAF3aPCgtNaVZG5zRvoIpVHsCDg=="
              crossorigin="anonymous">
      </script>
    </head>
    </html>
    ...
    public void LoadPluginSecure(string pluginPath) {
        ...
        var secureBasePath = Path.GetFullPath("/opt/myapp/plugins");
  ...
  <Tab title="PHP">
    ...
    * **Server-Side:** **Ensure `allow_url_include = Off`** in your production `php.ini` (this is the default and most critical defense against RFI). Never `include`/`require` paths constructed from user input; use an allow-list of local files (see `CWE-840` example). For auto-updates, download over HTTPS and verify signatures/hashes (`openssl_verify`, `hash_file`) before replacing files.
  ...
  <Tab title="Ruby">
    ...
    #### Mitigation and Best Practices
    * **Client-Side:** Add `integrity: true` (if using `javascript_include_tag` with newer Rails/sprockets) or manually add `integrity` and `crossorigin` attributes (if using plain tags or older Rails).
