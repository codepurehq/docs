...
# Terraform for multi-cloud setup
"""
# SECURE - Multi-cloud infrastructure as code
terraform {
  required_providers {
    aws = {
...
## Detection Patterns
  ...
  * Multiple cloud provider patterns in single file.
  * Configuration files with mixed cloud credentials.
  * Environment files with various cloud API keys.

## Prevention Best Practices

  1. Use native cloud identity services (IAM, Managed Identity, ADC).
  2. Implement centralized secret management (Vault, AWS Secrets Manager).
  3. Use cloud-native key management services.
  4. Implement least privilege across all clouds.
  5. Use separate credentials per environment.
  6. Enable MFA for all cloud accounts.
  7. Implement cloud security posture management (CSPM).
  8. Regular multi-cloud security audits.